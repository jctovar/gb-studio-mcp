    ...
    "list_assets",
    {
      ...
      inputSchema: {
        ...
        type: z
          .enum(ASSET_TYPES)
          .optional()
          .describe("Tipo de asset a listar. Si se omite, retorna todos los tipos."),
      },
    },
    handler(async ({ projectPath, type }: { projectPath: string; type?: AssetType }) => {
      ...
      const result = type
        ? { [type]: collect(type) }
        : Object.fromEntries(ASSET_TYPES.map((t) => [t, collect(t)]));
